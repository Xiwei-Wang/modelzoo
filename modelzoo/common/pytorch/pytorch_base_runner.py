...
class PyTorchBaseRunner(metaclass=abc.ABCMeta):
    ...
    def train(self, dataloader):
        ...
        """
        with self._configure_run(modes.TRAIN, dataloader):
            self.on_train_start()

            exit_training = False
            for epoch in range(self._num_epochs):
                exit_training = self.train_epoch(epoch, dataloader)
                if exit_training:
                    break
    ...
    def evaluate(
        ...
        Args:
            dataloader: A data loader for generating data to feed to the model.
        """
        with self._configure_run(modes.EVAL, dataloader):
            self.on_eval_start()
            self.eval_epoch(dataloader)
    ...
    def train_and_eval(
        ...
        Args:
            train_data_loader: A data loader for generating training data to
                feed to the model.
            eval_data_loader: A data loader for generating evaluation data to
                feed to the model.
        """
        with self._configure_run(
            modes.TRAIN_AND_EVAL, (train_data_loader, eval_data_loader)
        ):
            self.on_train_start()

            exit_training = False
            for epoch in range(self._num_epochs):
                exit_training = self.train_epoch(epoch, train_data_loader)

                self.eval_epoch(eval_data_loader)
    ...
    def create(
        ...
        use_cs = bool(cs_ip or compile_only)
