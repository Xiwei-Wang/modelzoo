...
from modelzoo.transformers.pytorch.huggingface_common.configuration_gpt2 import (
    GPT2Config,
)
...
class GPT2Attention(nn.Module):
    def __init__(self, config, is_cross_attention=False, layer_idx=None):
        ...
        self.c_proj = nn.Linear(
            self.embed_dim,
            self.embed_dim,
            bias=config.use_ffn_bias_in_attention,
        )
    ...
    def _attn(
        ...
        if self.scale_attn_weights:
            query = query * torch.tensor(
                1 / float(value.size(-1)) ** 0.5, dtype=torch.float16,
            )
...
class GPT2MLP(nn.Module):
    def __init__(self, intermediate_size, config):
        ...
        self.c_fc = nn.Linear(
            embed_dim, intermediate_size, bias=config.use_ffn_bias
        )
        self.c_proj = nn.Linear(
            intermediate_size, embed_dim, bias=config.use_ffn_bias
        )
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        if self.wpe and position_ids is not None:
        ...
        if self.wpe and position_ids is None:
        ...
        if attention_mask is not None:
            ...
            def _add_prefix_zero_mask_and_invert(
                attention_mask, extended_attention_mask, max_positions
            ):
                # in case past_key_values are used we need to add a prefix
                # zeros mask to the causal mask
                if attention_mask.shape[1] > max_positions:
                    prefix_seq_len = (
                ...
                attention_mask = extended_attention_mask * -1e4
            ...
            causal_attention_mask = _add_prefix_zero_mask_and_invert(
                causal_attention_mask, extended_attention_mask, max_positions
            )
...
class GPT2LMHeadModelLoss(nn.Module):
    def __init__(self, loss_weight=1.0):
        super(GPT2LMHeadModelLoss, self).__init__()
        self.loss_weight = loss_weight

    def forward(
        self, lm_logits, labels, vocab_size, attention_mask,
    ):
        loss_fct = CrossEntropyLoss(reduction='none')
        lm_loss = loss_fct(
            lm_logits.view(-1, vocab_size), labels.view(-1).long(),
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config, loss_weight=1.0):
        ...
        self.lm_head = nn.Linear(
            config.n_embd, config.vocab_size, bias=config.use_bias_in_output
        )
        self.loss_fn = GPT2LMHeadModelLoss(loss_weight)
    ...
    def forward(
        ...
        if labels is not None:
            loss = self.loss_fn(
                lm_logits, labels, self.config.vocab_size, attention_mask,
            )
